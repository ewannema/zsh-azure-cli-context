...
test_case "azctx use - switches to previous context"
setup
azctx new context1 >/dev/null 2>&1
azctx new context2 >/dev/null 2>&1
azctx use context1 >/dev/null 2>&1
azctx use context2 >/dev/null 2>&1
azctx use - >/dev/null 2>&1
if [[ "$AZURE_CONFIG_DIR" == "${ZSH_AZCTX_CONTEXTS_DIR}/context1" ]]; then
  pass
else
  fail "Expected to switch back to context1"
fi
teardown

test_case "azctx use - rejects when no previous context"
setup
azctx new test-context >/dev/null 2>&1
run_command azctx use -
if [[ $CMD_EXIT -ne 0 ]] && [[ "$CMD_OUTPUT" == *"no previous context"* ]]; then
  pass
else
  fail "Expected error about no previous context"
fi
teardown

test_case "azctx use - rejects when previous context no longer exists"
setup
azctx new context1 >/dev/null 2>&1
azctx new context2 >/dev/null 2>&1
azctx use context1 >/dev/null 2>&1
azctx use context2 >/dev/null 2>&1
azctx rm context1 >/dev/null 2>&1
run_command azctx use -
if [[ $CMD_EXIT -ne 0 ]] && [[ "$CMD_OUTPUT" == *"no longer exists"* ]]; then
  pass
else
  fail "Expected error about previous context no longer existing"
fi
teardown

test_case "azctx use - can switch back and forth"
setup
azctx new context1 >/dev/null 2>&1
azctx new context2 >/dev/null 2>&1
azctx use context1 >/dev/null 2>&1
...
azctx use - >/dev/null 2>&1
azctx use - >/dev/null 2>&1
if [[ "$AZURE_CONFIG_DIR" == "${ZSH_AZCTX_CONTEXTS_DIR}/context2" ]]; then
  pass
else
  fail "Expected to switch back to context2"
fi
teardown

test_case "azctx reset clears previous context"
setup
azctx new context1 >/dev/null 2>&1
azctx new context2 >/dev/null 2>&1
azctx use context1 >/dev/null 2>&1
...
if [[ $CMD_EXIT -ne 0 ]] && [[ "$CMD_OUTPUT" == *"no previous context"* ]]; then
  ...
  fail "Expected previous context to be cleared after reset"
fi
teardown
...
if [[ $CMD_EXIT -eq 0 ]] && [[ "$CMD_OUTPUT" == *"* test-context"* ]]; then
  ...
  fail "Expected active context to be marked with *"
fi
teardown

test_case "azctx list shows no asterisk when no active context"
setup
azctx new test-context >/dev/null 2>&1
run_command azctx list
if [[ $CMD_EXIT -eq 0 ]] && [[ "$CMD_OUTPUT" != *"*"* ]]; then
  pass
else
  fail "Expected no asterisk when no active context"
fi
teardown

test_case "azctx list marks active context with asterisk"
setup
azctx new context1 >/dev/null 2>&1
azctx new context2 >/dev/null 2>&1
azctx use context1 >/dev/null 2>&1
run_command azctx list
if [[ $CMD_EXIT -eq 0 ]] && [[ "$CMD_OUTPUT" == *"* context1"* ]]; then
  pass
else
  fail "Expected '* context1' in output"
fi
teardown
